...
This module provides the :class:`slideflow.mosaic.Mosaic` class, which plots tile images onto a map of slides,
...
They are created by first generating whole-dataset activations (using
:class:`slideflow.activations.ActivationsVisualizer`), which are then mapped into two-dimensional space using UMAP
dimensionality reduction (:class:`slideflow.statistics.SlideMap`). This resulting SlideMap is then passed to
:class:`slideflow.mosaic.Mosaic`, which overlays tile images onto the dimensionality-reduced slide map.
