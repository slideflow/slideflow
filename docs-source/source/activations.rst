...
To work more directly with layer activations, use the :class:`slideflow.activations.ActivationsVisualizer` class. Instancing the class supervises the calculation and caching of layer activations, which can then be exported, viewed (as a mosaic map), or analyzed with various statistical methods. ``Project.generate_activations()`` automatically creates and returns an instance of this class.
