...
class SlideViewer(Viewer):
    ...
    def __init__(self, wsi: sf.WSI, *args, **kwargs) -> None:
        ...
        print(f"[TIMING] SlideViewer._refresh_view_full (init): {time.perf_counter() - _t_rfv:.3f}s")
        _t_rr = time.perf_counter()
        self.refresh_rois()
        print(f"[TIMING] SlideViewer.refresh_rois (init): {time.perf_counter() - _t_rr:.3f}s")
    ...
    def _refresh_view_full(self, view_params: Optional[EasyDict] = None):
        ...
        _t_rfv_start = time.perf_counter()
        _caller = ''.join(_tb.format_stack(limit=4)[:-1]).replace('\n', ' | ')
        print(f"[TIMING] _refresh_view_full CALLED  caller={_caller}")
        ...
        _t_pyramid = time.perf_counter()
        self.view = self._read_from_pyramid(**view_params)
        print(f"[TIMING]   _read_from_pyramid: {time.perf_counter() - _t_pyramid:.3f}s  "
              f"(target={view_params.get('target_size')})")
        ...
        _t_rr = time.perf_counter()
        self.refresh_rois()
        print(f"[TIMING]   _refresh_view_full->refresh_rois: {time.perf_counter() - _t_rr:.3f}s  "
              f"total_rfv={time.perf_counter() - _t_rfv_start:.3f}s")
        self._update_view_timer()
    ...
    def refresh_rois(self) -> None:
        ...
        _t_rois_loop = time.perf_counter()
        _n_vbos = 0
        _t_vbo_total = 0.0
        _t_scale_total = 0.0
        _t_triangle_total = 0.0
        for roi_idx, roi in enumerate(self.wsi.rois):
            ...
            if fill:
                ...
                _t_triangle_total += time.perf_counter() - _t_tri

        _t_loop_total = time.perf_counter() - _t_rois_loop
        print(f"[TIMING]   refresh_rois loop: {_t_loop_total:.3f}s  "
              f"({_n_vbos} VBOs, scale={_t_scale_total:.3f}s, "
              f"vbo_create={_t_vbo_total:.3f}s, triangles={_t_triangle_total:.3f}s)")
    ...
    def render(self, max_w: int, max_h: int) -> None:
        ...
        if self._tex_obj is not None:
            ...
            if not self._first_render_done:
                ...
                print(f"[TIMING] *** SLIDE APPEARS ON SCREEN: {time.perf_counter() - self._first_render_t0:.3f}s after SlideViewer created ***")
