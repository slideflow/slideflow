...
class MultiModal_Mixed_Attention_MIL(nn.Module):
    ...
    def forward(self, x, modality_mask, return_attention=False):
        """Forward pass through the network.
        
        Args:
            x: Tensor of shape (batch_size, max_feat_len, n_modalities)
               containing all modality vectors padded to max length
            modality_mask: Tensor of shape (batch_size, n_modalities) indicating which
                         modalities are present (1) or missing (0)
        ...
        for i in range(self.n_modalities):
            # Extract relevant features for this modality
            modality_features = x[:, :self.n_feats[i], i]
            # Encode to shared space
            embedded = self.encoders[i](modality_features)  # Shape: (batch_size, z_dim)
