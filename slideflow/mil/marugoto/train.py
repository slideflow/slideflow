...
def train_mil(
    ...
    train_dl = DataLoader(train_dataset, batch_size=64, shuffle=True, num_workers=1, drop_last=True)

    val_dataset = marugoto.data.build_dataset(bags[val_idx], targets[val_idx], encoder=encoder, bag_size=None)
    val_dl = DataLoader(val_dataset, batch_size=1, shuffle=False, num_workers=os.cpu_count())

    batch = train_dl.one_batch()

    model = Marugoto_MIL(batch[0].shape[-1], batch[-1].shape[-1])
