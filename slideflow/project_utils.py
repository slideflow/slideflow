...
def ensemble_train_predictions(ensemble_path: str, hp_list: Optional[List]) -> None:
    ...
    Keyword Args:
        hp_list (list, optional): 2D list with hp name as key and ModelParams
            as value. If included, the ensemble is for a hyper deep ensemble model
    """
    ...
    for kfold_dir in kfold_dirs:
        for epoch in epochs:
            for member_id, member_path in enumerate(member_paths):
                ...
                # Modifying kfold_dir in case of Hyper Deep Ensemble
                if hp_list:
                    kfold_split = kfold_dir.split('HP0')
                    kfold_dir_new = f"{kfold_split[0]}{hp_list[member_id][0]}{kfold_split[1]}"
                    kfold_path = join(member_path, kfold_dir_new)        
                else:
