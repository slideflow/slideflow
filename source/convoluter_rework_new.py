...
# python3 convoluter_rework_new.py -m /Users/james/histcon/models/active -i /Users/james/thyroid/test -o /Users/james/thyroid/test -s 128 -c 2 -b 2 --fp16 --final --export
...
class Convoluter:
	...
	def build_model(self, model_dir):
		...
		with tf.Graph().as_default() as g:
			...
			with tf.name_scope('dataset_input'):
				tile_dataset = tf.data.Dataset.from_generator(self.gen_slice_wrapper, (self.DTYPE, tf.int64, tf.bool))
				...
				self.next_batch_images, self.next_batch_labels, self.next_batch_unique  = tile_iterator.get_next()

				# Generate ops that will convert batch of coordinates to extracted & processed image patches from whole-slide-image
				image_patches = tf.map_fn(lambda patch: tf.cast(tf.image.per_image_standardization(patch), self.DTYPE), self.next_batch_images)
			...
			self.prelogits = end_points['PreLogitsFlatten']
			self.slogits = end_points['Predictions']
			num_tensors_final_layer = self.prelogits.get_shape().as_list()[1]
			vars_to_restore = []
	...
	def create_generator(self, image_path, case_name, export_tiles):
		...
		window_stride = [int(self.SIZE/self.STRIDE_DIV), int(self.SIZE/self.STRIDE_DIV)]
		...
		stride = int(self.SIZE/self.STRIDE_DIV)
		self.x_logits_len = int(self.X_SIZE / stride)+1
		self.y_logits_len = int(self.Y_SIZE / stride)+1
		self.total_logits_count = x_logits_len * y_logits_len

		if self.total_logits_count != len(self.coord):
		...
		for y in range(0, (shape[0]+1) - window_size[0], window_stride[0]):
			for x in range(0, (shape[1]+1) - window_size[1], window_stride[1]):
				...
				if (y % self.SIZE == 0) and (x % self.SIZE == 0):
					...
					self.coord.append([y, x, True])
				else:
					self.coord.append([y, x, False])
	...
	def gen_slice_wrapper(self):
		yield from self.gen_slice
	...
	def scan_image(self, image_path, case_name, export_tiles=False, final_layer=False, save_pkl=True):
		...
		self.gen_slice = create_generator(image_path, case_name, export_tiles)

		with tf.Graph().as_default() as g:
			with tf.Session() as sess:
				count = 0
				prelogits_arr, logits_arr, labels_arr, unique_arr = [], [], [], []

				while True:
					try:
						count = min(count, self.total_logits_count)
						progress_bar.bar(count, self.total_logits_count, text = "Calculated {} images out of {}. "
																			.format(min(count, self.total_logits_count),
																			 self.total_logits_count))
						if final_layer:
							new_logits, new_prelogits, new_labels, new_unique = sess.run([tf.cast(self.slogits, tf.float32),
																			  tf.cast(self.prelogits, tf.float32),
																			  self.next_batch_labels,
																			  self.next_batch_unique])
							...
							new_logits, new_labels = sess.run([tf.cast(self.slogits, tf.float32), self.next_batch_labels])
			...
			logits_arr = logits_arr[0:self.total_logits_count]
			labels_arr = labels_arr[0:self.total_logits_count]
			...
			if final_layer:
				prelogits_arr = prelogits_arr[0:self.total_logits_count]
				prelogits_arr = prelogits_arr[sorted_indices]
				unique_arr = unique_arr[0:self.total_logits_count]
			...
			if final_layer:
				...
				prelogits_out = None
			logits_out = np.resize(logits_arr, [self.y_logits_len, self.x_logits_len, self.NUM_CLASSES])
			if save_pkl:
