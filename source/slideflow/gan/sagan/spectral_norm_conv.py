...
class ConvSN2D(Conv2D):
    ...
    def build(self, input_shape, dtype=tf.float16):
        if self.data_format == 'channels_first':
            channel_axis = 1
        else:
            channel_axis = -1
    ...
    def call(self, inputs, training=None):
        ...
        if training in {0, False}:
            ...
            with tf.control_dependencies([self.u.assign(K.cast(_u, dtype=tf.float32))]): # Added for mixed precision compatibility
                W_bar = K.reshape(W_bar, W_shape)
...
class ConvSN2DTranspose(Conv2DTranspose):
    ...
    def call(self, inputs):
        ...
        if training in {0, False}:
            ...
            with tf.control_dependencies([self.u.assign(tf.cast(_u, dtype=tf.float32))]):
                W_bar = K.reshape(W_bar, W_shape)
        self.kernel = W_bar
