...
class SlideFlowProject:
	...
	def extract_tiles(self):
		if self.get_task('extract_tiles') == 'complete':
			...
			print('Tile extraction already in process; restart? [y/N] ')
			sys.exit()
		...
		slide_list = glob(join(self.SLIDES_DIR, '**!(thumbs)/*.svs'))
		slide_list.extend(glob(join(self.SLIDES_DIR, '*.svs')))
		slide_list.extend(glob(join(self.SLIDES_DIR, '**!(thumbs)/*.jpg')))
	...
	def create_project(self):
		...
		if self.yes_no_input("Will models utilize pre-training? [y/N] ", default=False):
