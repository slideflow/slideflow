...
class Packer:
	...
	def subdivide_all_annotations(self, background_label = "case", ignore_background = True):
		...
		for index, shape in enumerate(self.data):
			...
			if label == background_label and not ignore_background:
				...
				squares, coordinates = self.tile_iterator(area_small, index, tile_screening_size)
	...
	def export_all_tiles(self, export_dir):
		...
		for index, shape in enumerate(self.data):
			...
			with warnings.catch_warnings():
				...
				self.place_tiles(area, offset, self.size, export_dir, im, index, max_squares, True)
	...
	def place_tiles(self, area, offset, tile_size, export_dir = None, image = None, _id = None, max_squares = None, graph = False, exclusions = None, color='g'):
		...
		for j in range(int(y_range/tile_size)):
			for i in range(int(x_range/tile_size)):
				...
				if areaPoly.contains(sTestSquare): 
					...
					if image:
						if max_squares:
							print("Subdividing annotation #%s: " % _id)
...
if __name__==('__main__'):
	...
	for case_id in cases:
		with open (join(DIRECTORY, "%s.json" % case_id)) as case:
			...
			export_dir = join(DIRECTORY, str(SIZE), case_id)
			if not os.path.exists(export_dir): os.makedirs(export_dir)

			case_data = json.load(case)

			packer = Packer(case_data, case_id, FACTOR, SIZE, full_image)
			packer.subdivide_all_annotations()
			packer.export_all_tiles(export_dir)
			packer.show_graph()