    ...
    def add_model(self, model):
        ...
        if group:
            ...
            if not self.outcome_labels:
                ...
                log.warning("WARNING: added a group to a subset with mismatching outcome labels or model type")
...
class Model:
    ...
    def get_predictions(self, epoch, level='patient'):
        if level not in ('patient', 'slide', 'tile'):
            log.error(f"Unknown prediction level {level}; must be 'patient', 'slide', or 'tile'.")
            return None
