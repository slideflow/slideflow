...
class Mosaic:
	...
	def __init__(self, args):
		...
		self.mapping_method = 'expanded' if args.expanded else 'strict'
		self.focus = args.focus
		self.initiate_figure()
		self.tile_zoom_factor = args.tile_zoom
		self.load_metadata(args.meta)
		if args.bookmark:
			self.load_bookmark_state(args.bookmark)
		elif args.pkl:
			self.load_umap_pkl(args.pkl)
	...
	def load_umap_pkl(self, path):
		print("[Core] Loading UMAP PKL weights and plotting points...")
		with open(path, 'rb') as umap_file:
			weights = pickle.load(umap_file)
			points_x = []
			points_y = []
			point_index = 0
...
def get_args():
	...
	parser.add_argument('-p', '--pkl', help='Path to saved UMAP weights in PKL format.')
	...
	parser.add_argument('-z', '--tile_zoom', type=int, default=15, help='(Optional) How lenient the algorithm should be when placing tiles ; default is 1.5.')
	parser.add_argument('--focus', type=str, help='(Optional) Focus on slides from a specific category')
	parser.add_argument('--um', type=float, help='(Necessary if plotting SVS) Size of extracted image tiles in microns.')
	parser.add_argument('--expanded', action="store_true", help='Generates mosaics with expanded placement of tiles instead of strict.')
