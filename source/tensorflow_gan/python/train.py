...
def stargan_model(generator_fn,
				  ...
				  input_feature_tensors,
				  reconstructed_feature_tensors,
                  generator_scope='Generator',
                  discriminator_scope='Discriminator'):
  ...
  feature_input_data = input_feature_tensors
  ...
  with tf.compat.v1.variable_scope(generator_scope) as generator_scope:
    generated_image = generator_fn(input_data) #generated_data_domain_target)
  ...
  with tf.compat.v1.variable_scope(generator_scope, reuse=True):
    reconstructed_data = generator_fn(generated_image)

  # Get reconstructed feature output data
  reconstructed_feature_data = reconstructed_feature_tensors
  ...
  with tf.compat.v1.variable_scope(discriminator_scope) as discriminator_scope:
    disc_gen_data_source_pred = discriminator_fn(
        generated_image) #num_domains
  ...
  with tf.compat.v1.variable_scope(discriminator_scope, reuse=True):
    disc_input_data_source_pred = discriminator_fn(
  ...
  # Placeholders
  disc_input_data_domain_pred = None
  disc_gen_data_domain_pred = None
  ...
  return namedtuples.StarGANModel(
      ...
      generated_data=generated_image,
      reconstructed_data=reconstructed_data,
	  reconstructed_feature_data=reconstructed_feature_data,
...
def stargan_loss(
  ...
  # Modified reconstruction loss wrt. input feature maps
  reconstruction_loss = None
  for feature_input, reconstructed_feature in zip(model.feature_input_data, model.reconstructed_feature_data):
    print(feature_input.shape, reconstructed_feature.shape)
    if reconstruction_loss is None: 
      reconstruction_loss = reconstruction_loss_fn(feature_input, reconstructed_feature)
    else:
      reconstruction_loss += reconstruction_loss_fn(feature_input, reconstructed_feature)
  ...
  # Classification Loss. - replaced with reconstruction loss
...
def gan_train(train_ops,
  ...
  with tf.compat.v1.train.MonitoredTrainingSession(
    ...
    while not session.should_stop():
      gstep = session.run(train_ops.global_step_inc_op, feed_dict={generator.get_layer('noise_input'):np.random.uniform(size=(2, 100))})
  return gstep
